...
@ddt
class EditUsersTypeTestCase(unittest.TestCase):
    # 拼接完整的excel路径，然后读取excel数据
    wb = ReadExcel(os.path.join(DATA_DIR, file_name), "edit_user_type")
    ...
    def test_edit_user_type(self, case):
        ...
        # 向接口发送请求
        response = self.request.request(method=case.method, url=url, headers=headers)
        time.sleep(2)
